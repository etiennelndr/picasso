...
def generator(config: Config, stage: Stage):
    ...
    for _ in range(steps):
        ...
        for j in range(config.batch_size):
            ...
            x_img = tf.keras.preprocessing.image.load_img(
                x_file, target_size=config.input_shape
            )
            y_img = tf.keras.preprocessing.image.load_img(
                y_file, target_size=config.output_shape
            )
